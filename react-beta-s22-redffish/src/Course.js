...
class Course extends React.Component {
  ...
  getCourseButton() {
    ...
    return (
      <Button className="custom-btn" size="sm" onClick={buttonOnClick}>
        {buttonText}
      </Button>
    );
  }
  ...
  render() {
    ...
    return (
      ...
      <Card className="d-flex mb-2 p-2" style={{backgroundColor:"#F8F8F8"}}>
        ...
        <Card.Subtitle className="d-flex mb-2 text-muted">
          <div className="flex-grow-1">{course} · {credits + " Credits"}</div>
          {!this.props.ratingMode && (
            <Button size="sm" style={{backgroundColor:"#0379A8", fontWeight:500}} onClick={() => this.openModal()}>
              View sections
            </Button>
          )}
          {this.getExpansionButton()}
        ...
        {/* {!this.props.ratingMode && (
          <Button className="mt-2" style={{backgroundColor:"#0379A8"}} onClick={() => this.openModal()}>
